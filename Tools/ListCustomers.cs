...
[McpServerToolType]
public static class ListCustomersTool
{
    [McpServerTool, Description("Lists all Lexoffice contacts with the customer role. Returns the raw JSON response.")]
    public static async Task<string> ListCustomersAsync(IServiceProvider serviceProvider)
    {
        var httpFactory = serviceProvider.GetRequiredService<IHttpClientFactory>();
        var client = httpFactory.CreateClient("LexOfficeClient");
        // Lexoffice API: GET /contacts?role=customer
        var response = await client.GetAsync("contacts?role=customer");
        switch ((int)response.StatusCode)
        {
            ...
            case 204:
                return "No Content: No customers found.";
            case 400:
